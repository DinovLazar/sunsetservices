...
The original M.03 design assumed Erick Valle, or a native Spanish-speaking designate, would read the Spanish site and Cowork would apply corrections. Goran decided in Chat that no native reviewer is queued for the launch window. Instead, Codex executes M.03 as the highest-quality LLM-driven Spanish review pass possible against the locked `Sunset-Services-TRANSLATION_NOTES.md` §M.01f1 glossary and register matrix.

This means launch ships with first-pass Spanish that has not been read by a human native speaker. The launch acceptance criterion in `Sunset-Services-Plan.md` §14 ("native Spanish review complete") is therefore technically unmet at launch. Goran accepted this trade-off explicitly, and M.03 closes anyway as an LLM review, not as a substitute for native judgment.
...
- Glossary consistency against §M.01f1, including the 4 division names, 28 service names, common terms, and brand/proper nouns.
