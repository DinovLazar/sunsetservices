...
- A11y / BP / SEO on `/contact/` all clear the plan's ≥95 cutoff after the `role="region"` fix (commit `aa6b399`). Prior to the fix, `/contact/` A11y was 94 (failed `aria-prohibited-attr` on the widget div).
...
4. **A11y `role="region"` fix.** Lighthouse `aria-prohibited-attr` flagged the widget `<div>`'s `aria-label` because plain `<div>` without an explicit role can't carry `aria-label`. Added `role="region"` to make the labelled landmark explicit. A11y on `/contact/` desktop went 94 → 97 after this fix. The fix is a single-line addition in `CalendlyEmbed.tsx` (commit `aa6b399`).
...
- [x] Vercel preview deployment in `READY` state on the final push (`sunsetservices-a6b1nc7qq-dinovlazars-projects.vercel.app`, commit `aa6b399`).
...
6. ✅ Build green at 118 pages, Vercel preview `READY` on commit `aa6b399`.
...
| SHA | Message |
|---|---|
| `f82aac4` | `chore(decisions): log Phase 2.07 decisions` |
| `34af242` | `chore(env): document Phase 2.07 Calendly variables` |
| `970b1b7` | `feat(calendly): CalendlyEmbed component + consent gate + lazy load (Phase 2.07)` |
| `ad00895` | `chore(i18n): Phase 2.07 Calendly strings` |
| `85a2b17` | `feat(contact): swap Calendly placeholder for real embed (Phase 2.07)` |
| `78636a5` | `feat(thank-you): swap Calendly placeholder for real embed (Phase 2.07)` |
| `aa6b399` | `fix(calendly): role=region on widget div (a11y) (Phase 2.07)` |
| `90ad28c` | `chore(phase-2-07): project-state updates` |
