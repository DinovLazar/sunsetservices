...
## 2026-07-23 — Phase B.18 plan-of-record: reusable portfolio importer + first real project pages

Phase B.18 set out to publish the first two real hardscape jobs (Providence Pointe, Wilshire) as full project pages, and to leave behind a reusable importer so the remaining ~19 Drive job folders become a manifest edit rather than a phase. Below are the decisions baked into the importer, plus four ground-truth corrections discovered on execution.

### Ground-truth corrections (the code won over the brief)

- **The "M.18 / PSS-002 story layout" the brief targets does not exist in this repository.** Verified against the checkout on 2026-07-23: the `project` schema (`sanity/schemas/project.ts`) is the **legacy** shape (`title`, `narrativeHeading`, `narrative`, `materials[]`, `leadImage`/`leadAlt`, `gallery[galleryEntry]`, `hasBeforeAfter` + before/after, `facts{sqft,durationDays,crewSize}`, `seo`); `PROJECT_DETAIL_PROJECTION` (`sanity/lib/queries.ts:84`) selects only those; the detail page (`src/app/[locale]/projects/[slug]/page.tsx`) renders a fixed `Hero → Narrative → Gallery → Facts → BeforeAfter → Related → CTA` tree with **no `hasStory` gate**. None of `overview / site / approach / work / feature / result / durability / atAGlance / sitePhotos / materialsNote / testimonialStatement / faq / internalLinks / keywords` exist as schema fields, projection selections, components, or `project.story.*` i18n keys, and `git log --all` has no M.18/PSS-002/story commit. The brief's own referenced files `sanity/schemas/objects/galleryEntry.ts` and `projectBlocks.ts` are absent (galleryEntry is defined inline; projectBlocks was never created).
- **Decision:** the importer is built against the **actual legacy schema** (per project rule "if the state file disagrees with the code, the code wins", and the brief's own §3 "the data should fit the schema, not the reverse"). Building it verbatim to the brief's story-field manifest would write ~15 fields per project that the schema does not define and the page cannot render — the drafts would show as the empty legacy branch, the opposite of the goal. A true "story layout" remains a **prerequisite for a future phase** (build the schema fields + projection + components + i18n first); B.18's own out-of-scope list forbids creating them here. Decided by user (Goran) after the finding was surfaced with evidence.
- **This session's host could not perform the photo or write legs.** (1) The staged photos the brief expects at `C:\sunset-photos\import\providence-pointe\` and `…\wilshire-wojdyla\` are **not present** — this is a macOS host (`/Users/petarjakimov`), and the prior Cowork photo work lived on a Windows box; no equivalent folder exists anywhere on disk. (2) `.env.local` carries **read-only** public config with **no `SANITY_API_WRITE_TOKEN`**. Per the brief's §2 stop-conditions, **nothing was written to Sanity, no photos were processed, and the manifests were left as empty templates.** The importer, README, asset-lock scaffold, and decisions were committed so the write is reproducible from a committed state on the machine that has both inputs.

### Import-approach decisions (ratified from the brief, applied to the legacy schema)

- **D4 — Documents are written as Sanity drafts (`drafts.project-<slug>`); publishing is a separate, explicitly gated `--publish` run.** Drafts are invisible to the anonymous published read client the site uses, so nothing unverified reaches a visitor while a reviewer can still see the whole page in Studio. `--publish` is refused unless `confirmed.clientPhotoPermission === true` **and** `confirmed.citySlug` **and** `confirmed.year` are set; the gate that failed is printed. Publishing fires the Phase B.08 revalidate webhook — the script prints that reminder.
- **D5 — Slugs never contain a house number, street name, client surname, or city.** Privacy (BG-01 §9.2) + URL stability (the city is currently unconfirmed; a slug that later gains a city is a broken URL). The importer's manifest validator rejects a slug with a `\d{3,}` digit-run or any of the 24 city slugs as a substring. Describe the work, not the address.
- **D7 — Every processed image is auto-oriented (`.rotate()` first, from EXIF, before it is stripped), downscaled to max 2400 px on the long edge, re-encoded at JPEG quality 82 (mozjpeg), and stripped of ALL metadata** (no `withMetadata()`). These are iPhone photos of private homes whose EXIF carries the customer's GPS coordinates; that must never leave the machine. The importer re-reads a processed file and asserts zero `exif`/GPS survive, and **fails the run before uploading a single byte** if any does.
- **D9 — `featuredOnHome: false` on every imported project.** The homepage hero is a brand-critical slot chosen by Goran/Lazar; it is never a side effect of an import. Flipping it later is a one-line manifest change.
- **D10 — `hasBeforeAfter: true` only when a genuine matched pair exists** (same subject, comparable angle, before + after). A mismatched pair is worse than none; when no honest pair exists the flag is `false` and the "before" shots go into the gallery, where they belong.
- **D13 — 3D renders and iPad design screenshots are not photographs of built work** (BG-01 §9.1 "real projects only"). A render may appear only where its alt text plainly says it is a 3D design rendering; never as `leadImage`, never in a before/after pair. Classified from the `RENDER/` and `iPad/` source subfolders and suffixed `-render-` in the processed filename so they are never mistaken for as-built photos.
- **D3 (ratified) — a committed, idempotent importer + JSON manifests, not hand entry in Studio.** Deterministic `_id`; asset uploads keyed on the SHA-256 of the processed bytes and recorded in a committed lockfile (`docs/project-import/asset-lock.json`) so a re-run reuses the existing asset instead of duplicating it; a re-run with an unchanged manifest is a no-op (compares against the stored doc and skips the write, so `_rev` does not move). Dry-run is the default and writes nothing.

**Decided by:** user (Goran) — chose "build for the real schema" and "build code scaffolding now" after the four ground-truth corrections were surfaced. **Logged by:** Code, 2026-07-23, before any other change in the phase (decisions-first commit).
