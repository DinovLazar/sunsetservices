...
- [x] **Harnesses pass.** Clean `npm run build` exit 0, **202/202** prerendered, 0 TS errors · `lint` **0 errors** (9 pre-existing warnings) · `tsc --noEmit` **0** · `validate:mobile` **0 errors** · `validate:links` **0 hard failures (0 broken / 0 error, 294 pages crawled)** · `validate:seo` **0/0 (196 URLs + sitemap + robots)** · `validate:schema` **0/0 (24 URLs)** · `test:consent` **23/23**. ⚠ `validate:a11y` was **still mid-run at push time — Goran explicitly instructed "just push to main now" without waiting**. **Post-push outcome: the local a11y harness proved non-reproducible on this Mac** — the first run hung for ~2 h at 0 % CPU (killed), then three completed runs on the *identical build* returned **81 → 754 → 711 errors with per-URL failure sets that flip between runs** (e.g. `/` scored Lighthouse 100 + axe 0 in run 1, then 52 errors in run 2). The artifacts are environment-class, not code-class: Lighthouse `PROTOCOL_TIMEOUT`/`NO_FCP` runtime crashes; axe scanning the `#__next_error__` shell (the SEO validator's 196-URL 0/0 title check + the 294-page link crawl prove those pages render fine); and identical shared-chrome target-size/contrast clusters (hidden `tabindex="-1"` mega-menu links, text measured against unloaded images) appearing on pages this phase never touched — the audit's fixed URL sample doesn't even include the 12 wired pages. This validator has a documented decode-race history (Hand-off B patched one). **Follow-up: re-run `validate:a11y` from a healthy machine or CI (`BASE_URL=<prod>`), expected to restore the B-15 0/0 baseline** — no B-16 surface is implicated (the phase adds descriptive alts, a min-height, and a text-shadow; no color/structure changes).
...
5. ES `photoAlt` drafts + the waterproofing division-alt value await native review (§B.16).
6. **`validate:a11y` needs one green run from a healthy environment** (CI or another machine, `BASE_URL=<prod>`): the local harness on this Mac was non-reproducible post-push (three runs, three different failure sets on one build — full evidence in §3's harness item). Expected to restore the B-15 0/0 baseline; treat any failure that *reproduces identically across two clean runs* as real.
