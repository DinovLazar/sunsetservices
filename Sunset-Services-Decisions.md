...
- **Extraction destination locked.** `C:\sunset-photos\` (outside the SunSet-V2 repo). `extracted\` holds the merged Takeout tree; `quarantine\` holds any failed ZIPs (empty — none failed); `logs\` holds the progress log, the failed-ZIP log, the full media CSV (`06-media-files.csv`), the per-folder summary (`07-folder-summary.json`), and the sitewide summary (`08-summary.txt`).
- **Extracted DIRECTLY from D: — no C: copy of the ZIPs.** In-spec deviation from the prompt's "robocopy to `C:\sunset-photos\zips\` then extract" step. Windows/WinRAR read the ZIPs straight off D: (read-only) and wrote only the extracted tree to C:, which halved disk use (no 151 GB of redundant ZIP copies) and time. D:\ remained untouched. The `zips\` subfolder was therefore never populated; there is nothing to delete there. The extracted tree on C: is kept pending the user's decision after M.01c.
...
- **Duplicate `-057 (1)` resolution:** byte-identical to `-057.zip` (same SHA-256 `F5B367CC32C092714C02F339A1B4523185E7A5AF47F9F51EB3C3792CA26917D0`), so `-057 (1)` was skipped (not extracted).
...
Goran reviewed the site after Phase M.01c and asked for a real expansion of the site's information architecture. Phase M.01d ratifies the locked target state and ships the data foundation; Phase M.01e flips every visible surface; Phase M.01f wires live Google reviews and the first-pass ES on the new content.

**Locked decisions (all settled before M.01d execution; no questions deferred):**

1. **Four divisions, not three audiences.** `landscape`, `hardscape`, `waterproofing`, `snow-removal`. Replaces the previous 3-audience IA (residential / commercial / hardscape).
2. **Residential vs commercial moves into the quote wizard.** No longer a top-level IA distinction; becomes a radio inside Step 4 (contact info) per M.01e. M.01d does NOT touch the wizard code — only describes the locked target in `Sunset-Services-Plan.md` §11.
...
8. **22 cities get full detail pages.** New cities (18): `hinsdale`, `oak-brook`, `elmhurst`, `clarendon-hills`, `burr-ridge`, `western-springs`, `glen-ellyn`, `downers-grove`, `winfield`, `lombard`, `st-charles`, `geneva`, `south-elgin`, `elburn`, `north-aurora`, `oswego`, `yorkville`, `plainfield`. Existing 4 retained as detail pages: `aurora`, `naperville`, `wheaton`, `batavia`.
...
10. **Per-city `trust.projectsCompleted` numbers stay UNCHANGED** in `locations.ts`. They represent a 25-year business and cannot be honestly re-derived; Erick supplies real per-city numbers later. New cities ship with conservative placeholder values pending Erick's input.
11. **Homepage H1 = "Build your outdoor legacy".** Replaces the Phase 1.06 H1 ("Outdoor spaces, built to last 25+ years."). New H1, not a sub-tagline. ES counterpart: "Construye tu legado al aire libre" (`tú` register to match the surrounding home block, which is marketing/persona surface).
12. **Q&A page is a single page with 25 questions.** Within the 20–30 range. Five categories × 5 questions each: pricing, process, materials, service area, logistics. M.01d adds the i18n strings (`qa.*` namespace in both locales); M.01e builds the page itself.
13. **Project address: display-layer-only strip of leading street number.** `stripStreetNumber()` helper at the top of `src/app/[locale]/projects/[slug]/page.tsx`. Applied to the project's metadata title and breadcrumb title. Does NOT modify `src/data/projects.ts`. Does NOT touch Sanity. Sanity / data-file address keeps the full street number; only render output strips it. No-op for the current 12 placeholder projects (none start with digits); activates the moment M.01c's local uploader lands real project documents whose title or locality includes a leading number.
14. **No `[TBR]` markers on any new ES string** (post-B.01 convention). All new ES strings ship as straight LatAm Spanish first-pass. Native review folds into Phase M.03 alongside the broader ES pass.
15. **Tone register split.** `usted` for the new `qa.*` namespace (informational / transactional surface) and ES register decisions in legal-adjacent strings. `tú` for the new homepage H1 ES counterpart, the 14 new service detail pages' bilingual content (marketing/content surface), and the 18 new city `whyLocal` / hero strings. Each new namespace stays internally consistent with its surface type.

**Code in-phase decisions (off-spec from the plan, surfaced during execution):**

- **`LOCATION_SLUGS` stays at 6 entries through M.01d; new `ALL_LOCATION_SLUGS` exports the full 24.** The plan called for adding all 24 cities to `locations.ts`. The naive approach — appending 18 new slugs to `LOCATION_SLUGS` — would have surfaced them automatically in three places (sitemap.ts, service-areas/[city]/generateStaticParams, isLocationSlug type guard) and broken the M.01d "zero visible change" mandate. Resolved by keeping `LOCATION_SLUGS` as the original 6 (so `isLocationSlug('hinsdale')` returns false → /service-areas/hinsdale/ → 404 by design), introducing a new `ALL_LOCATION_SLUGS` constant for the full 24 (used by the automation pipeline whitelist), and a new `getVisibleLocations()` helper for consumers that want "the cities visible in M.01d." Three consumers updated: `sitemap.ts`, `service-areas/page.tsx` (schema ItemList), `CitiesGrid.tsx` (the index card grid), `ServiceAreaMap.tsx` (the SVG pins). All four now go through `LOCATION_SLUGS` or `getVisibleLocations()` instead of `LOCATIONS.map`. M.01e returns these to the full list.
- **`generateStaticParams` filters out new-division services lacking `audience`.** The `Service.audience` field becomes optional in M.01d (existing services keep it for backwards-compat; new Waterproofing + Snow Removal services omit it). The audience-aware `/[locale]/[audience]/[service]/page.tsx` route's `generateStaticParams` would have produced `{audience: undefined, service: <new-slug>}` pairs and either failed type-check or produced `/undefined/` URLs. Added a filter that skips services with no `audience`. Same treatment applied to `sitemap.ts`'s service-path enumeration and the `revalidation.ts` bulk-invalidate path for `/[audience]/[service]`.
- **Snow Removal "commercial" service renamed `commercial-snow-plowing`.** The candidate slug "snow-removal" would have produced `/snow-removal/snow-removal/` under the M.01e `/<division>/<slug>/` route shape. Renamed to `commercial-snow-plowing` to keep the URL clean and disambiguate it from the existing residential + commercial `snow-removal` audience-aware entries (both of which stay in `services.ts` through M.01d for backwards-compat).
- **New cities' `featuredServices` arrays use only existing audience-aware services** (Option B from the plan's Step 7). The Phase 1.14 location page consumer renders `featuredServices` cards by building `/<audience>/<slug>/` URLs from each entry's `audience` field. New Waterproofing + Snow Removal services have no `audience`, which would break the link rendering for new cities. Option B (the plan's recommended path) defers the consumer update — and the Waterproofing/Snow Removal showcase on city pages — to M.01e. M.01d's new cities therefore showcase the same 16 audience-aware services as the existing 6 do.
...
Phase B.11 wires the dormant `data-photo-upload-slot` placeholder on Step 3 (`src/components/wizard/WizardStep3Details.tsx:82-87` — empty `<div hidden aria-hidden="true">`) to a real Sanity-backed photo uploader. Visitors can attach up to 10 photos (10 MB each) across all three Step 3 groups (`residential | commercial | hardscape`). Files upload immediately to Sanity as they're picked; the asset references attach to both partial-lead and full-quote-lead docs; the existing branded `QuoteLeadAlertEmail` template renders a small thumbnail grid linking back to the full-size photos. After B.11 closes, Bucket B is fully done and the wizard is feature-complete (every Step 1→5 surface is real, no placeholders left).

**Prior B.11 work disposition.** A 2026-05-20 branch (`claude/suspicious-pare-6bc918`) carried a complete but never-merged B.11 implementation that pre-dated M.01e-pt2 (audience→group rename + localStorage `_v1`→`_v2` bump), M.01f1 (LatAm-MX Spanish glossary lock), and a handful of new D1–D27 requirements (AbortController on Remove-during-upload, chat-KV rate-limit piggyback, per-file retry). Chat decided 2026-05-27 to abandon that branch and start fresh against current `origin/main` so the new D1–D27 are honored cleanly. The prior worktree is left dormant; its code patterns may be referenced but its commits are not reused.

**Dependency-satisfied note (re-verified 2026-05-27 against `origin/main` HEAD `0985a77`).** Phase 1.20 placeholder slot is live (`WizardStep3Details.tsx:82-87`). Phase 2.05 Sanity write client is at `sanity/lib/write-client.ts` (the plan's wording said `writeClient.ts` — same module, hyphenated filename; cosmetic-only discrepancy). Phase 2.06 `/api/quote/route.ts` + `/api/quote/partial/route.ts` shipped. Phase 2.06 client-side session id helpers live in `src/lib/sessionId.ts`. Phase 2.08 `QuoteLeadAlertEmail` + `QuoteConfirmationEmail` branded templates + `sendBrandedEmail` utility live under `src/lib/email/templates/`. Phase 2.17 Sanity `client.assets.upload('image', buffer, {filename, contentType})` pattern is well-trodden in `src/lib/automation/portfolio/uploadPhotos.ts`. Phase B.10 added the `inputRef` prop to `WizardField`. Phase M.01e-pt2 storage key is `sunset_wizard_progress_v2` (confirmed at `src/lib/wizard/storage.ts:25`). AnalyticsBridge listens for `sunset:wizard-event` CustomEvents on `document` with the flat `detail: {name, ...payload}` shape (confirmed at `src/lib/wizard/events.ts:62`). Phase B.06/B.07/B.08/B.09/B.10 harnesses (`validate:schema|seo|a11y`, `test:wizard-autocomplete`) are wired in `package.json:22-25`.

**27 locked decisions (D1–D27) — settled by Chat 2026-05-27 before Code execution:**

**D1 — Storage backend: Sanity assets.** Photos land in the same Sanity dataset as project photos via `writeClient.assets.upload('image', buffer, {filename, contentType})`. Reuses the existing `SANITY_API_WRITE_TOKEN` from Phase 2.05 — no new env var required for storage. Rejected: Vercel Blob (one more vendor + separate quota; Sanity assets are the simpler choice and the Phase 2.17 portfolio pipeline already does the same thing).

**D2 — Upload timing: immediately on file pick.** Visitor picks N files → client POSTs each one (bounded parallelism, see D20) to a new `/api/quote/photo-upload` route → route validates + uploads to Sanity → returns `{assetId, url, dimensions, originalName, contentType}` → wizard state appends to `step3.photos[]`. Step 5 Submit just references the already-uploaded asset _ids by sending `photoAssetIds: string[]` to `/api/quote`. Partial-lead writes (the Step 1→2 / 2→3 fire-and-forget POSTs from Phase 2.06) carry `photoAssetIds: string[]` too so Erick sees photos from abandoners. Rejected: upload-on-submit (visitor would wait at Step 5 for the upload to complete; abandoners would never trigger any upload).

**D3 — Limits: 10 photos × 10 MB each per submission.** Server enforces per-file 10 MB cap (rejects oversized with 400 + opaque `too-large`). Server enforces 10-photo aggregate count by inspecting the existing `quoteLeadPartial`/`quoteLead` doc for this session BEFORE accepting the new asset (rejects with 400 + opaque `too-many`). Client mirrors both checks pre-flight for instant feedback.

**D4 — Optional, not required.** Visitors who don't want to upload still advance to Step 4. Many visitors won't have photos handy and forcing it would tank conversion. The Step 3 Next button is enabled regardless of photo count (existing required-field validation for the rest of Step 3 is unchanged).

**D5 — Universal across all three Step 3 groups.** The photo upload slot renders on `residential`, `commercial`, AND `hardscape` `WizardStep3Group` field-map branches. Don't gate by group — every group benefits from photos. The B.11 component mounts inside the existing Step 3 component, AFTER the group-conditional fields and BEFORE the Step 3 Next button.

**D6 — Allowed MIME types: JPEG, PNG, HEIC, WebP.** HEIC is critical — iPhones default to it. Both client-side `<input accept="...">` and server-side MIME sniff enforce. Rejected client-side HEIC→JPEG conversion (would need `heic2any` ~600 KB chunk; Sanity stores HEIC fine; email clients that don't render HEIC still get the link).

**D7 — Lead-alert email: thumbnail grid with links.** `QuoteLeadAlertEmail` gains a new optional `photos?: Array<{url, alt}>` prop. When present and non-empty, render a 3-col grid of ~120×120 px `<img>` thumbnails (Sanity's `?w=240&h=240&fit=crop` URL transform), each wrapped in `<a href={fullSizeUrl}>`. When absent or empty, the photos section doesn't render. Rejected: attaching the photos to the email (25 MB Gmail wall + slow inbox loads for Erick on mobile).

**D8 — Visitor confirmation email: one-line acknowledgment, no thumbnails.** `QuoteConfirmationEmail` gets a tiny "We received your N photos." line below the "thanks for reaching out" block when `photoCount > 0`. Pluralized via i18n. No thumbnails — the visitor already has the photos.

**D9 — Analytics: `wizard_photos_uploaded` event.** Informational, NOT a conversion. Payload `{step: 3, count: <number>}`. Fires ONCE per batch completion (after the last file in a multi-file pick resolves) — NOT per file. Dispatch via `fireWizardEvent(WIZARD_EVENTS.PHOTOS_UPLOADED, {step: 3, count})` per the existing Phase 2.10 / B.10 convention (`document.dispatchEvent` with flat `detail: {name, ...payload}`). Zero PII in payload — no file names, no MIME types, no sizes. Register in BOTH `src/lib/analytics/events.ts` (`ANALYTICS_EVENTS.WIZARD_PHOTOS_UPLOADED`) and `src/lib/wizard/events.ts` (`WIZARD_EVENTS.PHOTOS_UPLOADED`).

**D10 — UI: drag-and-drop + click-to-pick + thumbnail grid + per-file Remove.** Empty state shows a cream dropzone card with a dashed border, a centered cloud-upload Lucide icon, primary text ("Add photos of the project area"), helper text ("PNG, JPG, HEIC, or WebP — up to 10 photos, 10 MB each"), and a `.btn-secondary` "Choose photos" button. Populated state shows a 3-col grid (`grid-cols-2 sm:grid-cols-3`) of thumbnails (4:3 aspect, `object-cover`), each with a small `×` Remove affordance top-right (44×44 px hit target). Below the grid: muted-text count + size summary + "Add more" link affordance (hidden when at the 10-photo cap). One outer `<input type="file" multiple accept="image/jpeg,image/png,image/heic,image/webp" hidden>` driven by a ref.

**D11 — Per-file state machine.** Each `WizardPhoto` in `step3.photos[]` carries `{id, status, file?, assetId?, url?, dimensions?, errorReason?}`. `status` ∈ `'pending' | 'uploading' | 'ready' | 'error'`. `id` is a client-side `crypto.randomUUID()` for React keying + Remove targeting. `file` (the `File` object) lives in state during upload, gets cleared on success (replaced by `assetId` + `url`); on error, `file` stays so the visitor can retry by tapping the error thumbnail. Thumbnail rendering uses `URL.createObjectURL(file)` during upload and the Sanity CDN URL after.

**D12 — Per-file retry on error.** Click an `error`-status thumbnail → re-fires the upload for that one file. No exponential backoff; one retry per click. Don't retry automatically — the visitor stays in control.

**D13 — Remove during upload aborts the request.** Each in-flight upload gets its own `AbortController`; the per-file Remove handler calls `abort()` if status is `uploading`, then drops the entry from state regardless of status.

**D14 — PII boundary preserved.** `step3.photos` carries asset IDs + thumbnail URLs + dimensions; never name/email/phone/address. Step 4 is still the PII boundary (Step 4 state never persists to localStorage per the Phase 1.20 D9 rule). Step 3 photo state IS allowed in the `sunset_wizard_progress_v2` autosave under the `step3` branch — but ONLY the resolved `{assetId, url, dimensions, status: 'ready'}` rows, never the in-flight `file` objects or any in-flight metadata. Pending/uploading/error rows are stripped before serialization.

**D15 — Autosave shape.** Update the `WizardProgressV2` Zod schema in the wizard state library to accept `step3.photos` as `Array<{assetId: string, url: string, dimensions: {width, height}, status: 'ready'}>`. The `WizardPhoto` runtime type in the React layer is wider than the persisted shape — narrow before write, widen on read.

**D16 — `/api/quote/photo-upload` route — flag-gated, rate-limited, authenticated by session.** New POST route. `runtime = 'nodejs'`, `dynamic = 'force-dynamic'`. Gated by `WIZARD_PHOTO_UPLOAD_ENABLED` (default `true` once Vercel is updated; default `false` in tests that need the disabled fallback). Auth: payload includes `sessionId` from `getOrCreateSessionId()` (matches the Phase 2.06 partial-lead route's auth model — no Bearer token, just session-stickiness). Reads request as `multipart/form-data` (NOT `request.json()`) — single field `file` of type `File`. Validates: presence, size ≤ 10 MB, MIME ∈ {`image/jpeg`, `image/png`, `image/heic`, `image/webp`} via the buffer's magic bytes (NOT the client-supplied `Content-Type` — sniff via the first 12 bytes), aggregate photo count for this session ≤ 10. On success, uploads to Sanity via `writeClient.assets.upload('image', Buffer.from(arrayBuffer), {filename: ` + "`quote-photo-${randomUUID()}.${ext}`" + `, contentType: mime})` and returns 200 + `{assetId, url, dimensions: {width, height}, originalName, contentType}` (the Sanity SDK returns `_id` + `url` + `metadata.dimensions`). Failures: 400 + opaque `{status:'error', reason:'too-large' | 'wrong-type' | 'too-many' | 'invalid-payload'}`; 503 + `{status:'simulated', reason:'feature-flag'}` when flag-off; 500 + opaque `{status:'error', reason:'upload-failed'}` when Sanity throws (logged server-side, NOT echoed). Same opaque-body pattern as Phase 2.06 / 2.13.

**D17 — Rate limit: piggyback the existing chat KV limiter.** Reuse `checkRateLimit(ip)` from `src/lib/chat/rateLimit.ts` BUT under a different key prefix — `photo-upload:burst:<ip>` (2-second burst window) and `photo-upload:daily:<ip>` (50 uploads/day). Either extend the existing module with a `scope` parameter (e.g. `checkRateLimit(ip, 'photo-upload')`) OR create a thin wrapper in `src/lib/quote/photoUploadRateLimit.ts`. Pick whichever is the smaller diff (the existing module currently hard-codes the key prefixes; a tiny refactor + parameter is probably cleaner than a wrapper). Note: chat rate limiter is on `'memory'` mode in Vercel currently — same constraint applies here; cold starts reset counters; Cowork's Upstash flip in Phase B.09 follow-up benefits both surfaces.

**D18 — No virus scanning, no EXIF stripping, no re-encoding server-side.** Out of scope. Sanity's CDN serves the originals; if Erick later wants EXIF GPS stripped from public photos, that's a Phase M.x add-on. Note: lead-alert + visitor confirmation emails are private (sent to Erick + the lead's own email) so EXIF on those is a non-issue; project portfolio photos that DO go public are separately curated by Cowork.

**D19 — No Sanity asset metadata enrichment beyond defaults.** Sanity's `assets.upload` auto-extracts dimensions + content hash + content type — that's all we need. No custom `title`, `description`, `altText`, or tags on the asset itself (those would belong on the parent `quoteLead`/`quoteLeadPartial` doc, not the raw asset).

**D20 — Bounded client-side parallelism.** When a visitor picks N files at once, upload them in parallel with a concurrency cap of 3. Use a simple promise-pool pattern (no external dep). Reduces the chance of mass network failures on cellular connections without making the wizard feel sluggish.

**D21 — `quoteLead` + `quoteLeadPartial` schema additions.** Both schemas gain a new `photos` field — array of `{type: 'reference', to: [{type: 'sanity.imageAsset'}]}` (NOT a `weakReference: true`; the reference must be strong so Sanity's "Used by..." surface shows the lead doc). The schemas already have a `meta` field group — add `photos` under a new `media` field group (or under `meta` if the project's existing pattern is to keep secondary content in `meta` — match the existing convention, don't invent a new one). Order the field to render in Studio after the project-details fields and before the meta block. Sanity Studio redeploy (`npm run studio:deploy`) is part of the phase.

**D22 — Studio-side preview: image grid via custom preview component.** Add a Sanity preview component that renders the photos as a small thumbnail grid in the document list view's subtitle slot (or in a dedicated preview block on the form view). Keep it minimal — Sanity's default reference preview already does most of the work; only customize if the default looks broken. If it would take more than 30 minutes to nail, ship the default preview and log a follow-up.

**D23 — `/api/quote` + `/api/quote/partial` route changes.** Both routes get a new optional `photoAssetIds: z.array(z.string()).max(10).optional()` field on their Zod schemas (NOT individual asset uploads — those went through `/api/quote/photo-upload`). The routes resolve each ID into a `_ref` and set the `photos` field on the `client.create` / `client.patch.set` Sanity write. Hardening: validate each `assetId` matches the Sanity asset ID regex (`/^image-[a-f0-9]+-[a-z0-9]+-[a-z0-9]+$/`) before passing to the write client; reject the whole submit on regex mismatch (defensive — a tampered client could try to attach an arbitrary `_ref` like a project doc). Add a constant `SANITY_ASSET_ID_REGEX` to a shared location (`src/lib/sanity/assetId.ts` or similar).

**D24 — Lead-alert email rendering.** Pull the photo URLs into the email at send time. `sendQuoteLeadAlertEmail` (in `src/lib/quote/resend.ts` or wherever Phase 2.08 landed it) gains a `photos?: Array<{url, alt}>` parameter built from the asset IDs via a small helper that queries Sanity for the published assets and returns their CDN URLs. Pass to `<QuoteLeadAlertEmail photos={...} />`. Thumbnail src uses Sanity's `?w=240&h=240&fit=crop&q=80` URL transform; link href uses the bare CDN URL.

**D25 — Step 5 Review surface.** The review card on Step 5 gets a new "Photos" row when `step3.photos.length > 0` — small horizontal scroll of thumbnails (or wrap to 2 rows on mobile), no Edit link (Edit is on the Step 3 chip; clicking the chip jumps back to Step 3 where photos can be edited). When `photos.length === 0`, render "(no photos)" in muted text. Don't render the Photos row when no photos — keep the review tidy.

**D26 — EN + ES copy under `wizard.step3.photos.*`.** EN per the spec. ES in straight LatAm-MX Spanish, `usted` register, per M.01f1 §3 register matrix (Step 3 is informational/forms surface). Glossary picks: "fotos" (not "fotografías"); "estimado" if a CTA-like line appears (none should in this namespace — Step 3 doesn't have a CTA). No `[TBR]` prefix anywhere in shipped strings — post-B.01 convention.
...
- T1 Empty-state dropzone renders with correct ARIA (`role="region"` + `aria-label`, button is keyboard-focusable, label visible).
- T2 Happy path: upload one real test JPEG → 200 response → thumbnail appears in grid within 5 s → wizard state carries one `WizardPhoto` with valid `assetId` matching `SANITY_ASSET_ID_REGEX`.
- T3 Oversize: upload an 11 MB fixture → 400 `too-large` → error thumbnail in grid → Remove + retry both work.
- T4 Wrong type: upload a PDF fixture → 400 `wrong-type` → error thumbnail.
- T5 Aggregate cap: pre-seed state with 10 successful `ready` photos → pick an 11th → 400 `too-many` from the server (client-side guard ALSO blocks but T5 verifies the server-side check by bypassing the client-side check via direct fetch).
- T6 Per-batch analytics: pick 3 files at once → ONE `sunset:wizard-event` with `detail: {name: 'wizard_photos_uploaded', step: 3, count: 3}` (NOT three separate events).
...
- T8 Per-file retry: simulate one upload's network failure → error thumbnail → click error thumbnail → retry succeeds → thumbnail flips to `ready`.
...
- T10 Flag-off branch: spawn a second `next start` with `WIZARD_PHOTO_UPLOAD_ENABLED=false` → dropzone renders DISABLED state with a "photo upload is temporarily unavailable" helper line (EN + ES variants both checked).

Use `page.addInitScript` to mock Sanity's response in T2 / T8 so the harness never hits the real Sanity API (zero quota burn — same pattern as B.10's Google Places mock).

**Decided by:** Chat, 2026-05-27, before B.11 execution. D1–D27 are the input contract; any execution-time off-spec decisions (file name discrepancies, schema-shape convention picks where the plan offered options, etc.) append below this entry once Code surfaces them. The 2026-05-20 `claude/suspicious-pare-6bc918` worktree is abandoned per Chat's start-fresh decision.
...
- **M.11c-E1 — The baseline `validate:seo` "18 errors" was a STALE-LOCAL-SANITY-CACHE misdiagnosis, not content removal.** The local `next dev` server + local `next build` served stale Sanity content (3 blog posts 404'd, 3 projects missing from the *local* sitemap), so the harness's `BLOG_SLUGS`/`PROJECT_SLUGS` were reconciled down in good faith (operator-authorized, commit `a566e6b`). The Vercel Preview (authoritative, fresh prod content) then proved all **8 blog + 10 projects render 200 and appear in the sitemap** — matching the *original* M.11-reconciled lists. **Reverted (`603605a`); `validate:seo` = 0/0 on the Preview.** No site/content/Sanity change. Lesson: cross-check live/prod data before reconciling a content-coupled harness.
- **M.11c-E2 — Two error classes drove the fixes** (all else was clean or warning-level): (1) the **320px page cutoff on /contact + /thank-you** (the Calendly widget's flat `minWidth:320` + 16px container padding → +16px scroll) — the operator's core complaint; (2) the service-area map SVG pins rendering 16×16 (<24) on mobile.
...
- **M.11c-E5 — Map-pin <24 reclassified ERROR→WARNING in the harness** per WCAG 2.5.8's equivalent-control exception (the full-size CitiesGrid cards cover every city); pins also bumped 16→22. Documented in the harness + completion-report flag-and-log list.
- **M.11c-E6 — Merge path (per M.11c-D2):** merged `phase/m11c-mobile-sweep` into `main` (in the `main-integration` worktree) and pushed `origin main`, publishing M.10f + M.11b-link + M.10g + M.11c together (origin/main was 3 phases behind) — the operator's explicit push-everything authorization. (Merge SHA in the merge commit.)
