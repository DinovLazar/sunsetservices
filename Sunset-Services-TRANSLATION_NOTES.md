...
## Phase B.16 — Stock-bridge photo alt text (11 services + 1 division landing) (added 2026-07-04)

### Scope

Eleven service pages (4 Hardscape: `retaining-walls`, `fire-pits-features`, `driveways`, `outdoor-kitchens`; 6 Waterproofing: `basement-waterproofing`, `window-wells`, `crawl-spaces`, `concrete-raising`, `humidity-control`, `radon-mitigation`; 1 Trenchless: `pipe-fusing`) received real free-license bridge photos (B-16 sourcing) wired into the image pipeline this phase, and the `/waterproofing/` division landing got a dedicated bridge hero. Each service carries a descriptive `photoAlt` (`{en, es}`) on its `services.ts` entry (consumed by the service-detail hero + division-landing tile grid + city-page service grid, falling back to the service name elsewhere). The division-landing hero alt lives in i18n at `division.waterproofing.hero.alt` in `src/messages/{en,es}.json` (consumed by `[locale]/[division]/page.tsx` via `t('hero.alt')`) — the previous aspirational value was replaced because the new hero shows a different scene. The **EN alt is authoritative** in `docs/stock-bridge/stock-image-manifest.md` (used verbatim). The **ES below is a drafted translation** — same generic rules as the images: describe only what is in frame, never name Sunset, never name a city, never imply the work is Sunset's. `sump-pumps` remains a GAP (no image, no alt); `missile-boring` stays on the diagram track.

**Two sourced images were NOT wired (Code-phase verification failures, flagged to Chat):** the `/trenchless/` division-landing hero (readable "BARANGAY AYALA ALABANG / FIBER OPTIC PROJECT" shirt print geolocates the scene) and `handhole-pull-box` (readable Russian signage — "О ПОЖАРЕ ЗВОНИТЬ 101, 112" etc.). Both pages keep their previous imagery and alt strings; their drafted ES pairs are retained in the table below (greyed rows) in case replacement photos reuse the wording.

**Status: awaits native review** like other post-M.03 additions. Register: descriptive/neutral (image alt, not marketing copy). Terminology aligned to the locked service-name glossary: *muro de contención* (retaining wall), *fogata* (fire pit — matches the hardscape landing alt), *entrada de auto* (driveway — matches "Entradas de Auto"), *adoquines* (pavers), *hormigón* (concrete, §B-14), *conducto* (conduit, §B-14), *espacio bajo el piso* (crawl space — matches "Espacios Bajo el Piso"), *fusión a tope* (butt fusion).

### EN → ES alt pairs (12 wired + 2 unwired)

| Surface (slug) | EN (authoritative, from manifest) | ES (drafted, pending native review) |
| --- | --- | --- |
| `retaining-walls` | A curved segmental-block retaining wall with capstone coping and stone steps enclosing a circular paver patio, edged by shrubs and hedges. | Un muro de contención curvo de bloques segmentados con remate de albardilla y escalones de piedra que rodea un patio circular de adoquines, bordeado de arbustos y setos. |
| `fire-pits-features` | A built stone-clad rectangular fire pit on a flagstone patio, ringed by fixed wooden bench seating under a timber pergola. | Una fogata rectangular de obra revestida de piedra sobre un patio de lajas, rodeada de bancas de madera fijas bajo una pérgola de madera. |
| `driveways` | A wide interlocking-paver driveway with dark border insets leading up to the garage and entrance of a modern two-story home. | Una amplia entrada de auto de adoquines entrelazados con franjas de borde oscuras que conduce al garaje y la entrada de una casa moderna de dos pisos. |
| `outdoor-kitchens` | A built-in outdoor kitchen with a stainless grill, sink, and beverage fridge set into cabinetry under a wood-clad covered patio, beside a fenced lawn. | Una cocina exterior empotrada con parrilla de acero inoxidable, fregadero y refrigerador de bebidas integrados en los gabinetes, bajo un patio techado revestido de madera, junto a un césped cercado. |
| `basement-waterproofing` | Pale mineral streaks left by water tracking down the surface of a weathered concrete wall. | Vetas minerales pálidas dejadas por el agua al escurrir por la superficie de un muro de hormigón desgastado. |
| `window-wells` | Low windows with stone sills set near ground level in the brick exterior walls of a building light-well. | Ventanas bajas con alféizares de piedra situadas casi al nivel del suelo en las paredes exteriores de ladrillo del patio de luces de un edificio. |
| `crawl-spaces` | An open crawl-space access hatch at the base of a home's shingled exterior wall, with a person crouched beside the opening. | Una trampilla de acceso abierta al espacio bajo el piso en la base de la pared exterior con tejuelas de una casa, con una persona agachada junto a la abertura. |
| `concrete-raising` | A cracked and crazed concrete pavement surface strewn with fallen yellow flower petals. | Una superficie de pavimento de hormigón agrietada y cuarteada, cubierta de pétalos amarillos de flores caídos. |
| `humidity-control` | Heavy condensation and running water droplets covering the interior surface of a window pane. | Condensación intensa y gotas de agua escurriendo que cubren la superficie interior del cristal de una ventana. |
| `radon-mitigation` | A grey-white PVC vent pipe protruding from the white stucco exterior wall of a building against a clear blue sky. | Un tubo de ventilación de PVC gris blanquecino que sobresale de la pared exterior de estuco blanco de un edificio contra un cielo azul despejado. |
| _`handhole-pull-box` — **NOT wired** (image failed verification)_ | A ground-mounted grey utility power cabinet against a brick wall, with thick black conduits rising from the ground beside it. | Un gabinete eléctrico de servicios gris montado a nivel del suelo contra una pared de ladrillo, con gruesos conductos negros que emergen del suelo a su lado. |
| `pipe-fusing` | A gloved worker aligning a length of yellow polyethylene pipe clamped in a hydraulic butt-fusion machine for joining. | Un trabajador con guantes alineando un tramo de tubería de polietileno amarilla sujeta en una máquina hidráulica de fusión a tope para su unión. |
| `/waterproofing/` landing hero (`division.waterproofing.hero.alt`) | A clean, dry, well-lit finished basement with neutral painted walls, carpeted floor, and a staircase in the background. | Un sótano terminado, limpio, seco y bien iluminado, con paredes pintadas en tonos neutros, piso alfombrado y una escalera al fondo. |
| _`/trenchless/` landing hero — **NOT wired** (image failed verification; `division.trenchless.hero.alt` keeps its previous value)_ | A tracked horizontal directional drilling rig boring beneath a street while workers handle drill rods on the ground beside it. | Una perforadora direccional horizontal sobre orugas perforando bajo una calle mientras trabajadores manipulan barras de perforación en el suelo a su lado. |

### Open questions for native review (B-16)

- **`fogata`** (fire-pits-features) follows the existing hardscape-landing alt ("muro de asiento y fogata"); a reviewer may prefer *fogón* or *fosa de fuego* for a built-in masonry pit — kept as *fogata* for internal consistency.
- **`patio de luces`** (window-wells) renders "building light-well"; regional alternatives include *pozo de luz*. The service name itself is "Pozos de Ventana", so *pozo de luz* may read more consistently — flagged for the reviewer.
