...
| M.01d | Data + plan restructure | **Code** | 4-division shape (Landscape / Hardscape / Waterproofing / Snow Removal) + 22 detail-page cities + new H1 ("Build your outdoor legacy") + Q&A i18n (25 Q&A across 5 categories) + project-address render strip; zero visible site change other than the H1 and address format. Adds 14 new services and 18 new city entries to typed data files. M.01e is the visible flip. |
| M.01e | New pages + navigation flip | **Code** (+ small Design handover for Waterproofing icons) | Builds 10 Waterproofing service pages + 4 Snow Removal service pages + 4 division landings + 18 new city pages + the new `/qa/` page; deletes `/residential/*` + `/commercial/*` + `/service-areas/lisle/` + `/service-areas/bolingbrook/` with 301 redirects to `/` and `/service-areas/`; redraws the service-areas map; updates the navbar mega-panel; flips wizard Step 1 to the 4-division picker and adds the residential/commercial radio to Step 4; updates the homepage 3→4 division block. The biggest single phase in the project. |
| M.01f | Live Google reviews + ES first-pass for new content | **Code** | Wires the M.05 daily Google reviews fetcher (slotted forward from its original M.05 home) and replaces the placeholder testimonials on every location page with real reviews. Applies first-pass LatAm-MX ES to every new EN string shipped in M.01d + M.01e. Hands off to M.03 for native review. |
...
| M.03 | Codex-driven Spanish review pass (native review deferred) | **Code** | **Complete 2026-05-27.** Codex reviewed Spanish against the locked M.01f1 glossary/register matrix and applied targeted fixes in source + Sanity. Human native review remains deferred post-launch per the M.03 Decisions entry; no new phase row added here by user direction. |
...
**M-bucket dependency note.** Hard chains:
- **M.01d → M.01e → M.01f → M.02** is a hard chain. M.01d is data-only and ships zero visible change. M.01e is the visible flip and is the largest single phase. M.01f wires live reviews and primes M.03's ES review pool with the new EN strings from M.01d + M.01e.
- **M.04 → M.04a → M.05** is a hard chain.

M.07 (Mautic) is fully independent and can slot in anywhere. M.03 (Spanish review) runs in parallel from the moment Erick is available — its review pool grows when M.01f completes the first-pass ES on the new M.01d/M.01e content. Everything else is sequential.
