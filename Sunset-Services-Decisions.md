...
## 2026-07-04 — Phase B-16 Code: Hardscape + gap-services image integration + division-hero sizing fix (plan-of-record)

**B-16 Code plan-of-record.** Integrate the 14 B-16-Cowork bridge photos per the B-14/B-15 pipeline and fix the division-landing hero clipping. Scope: **12 service pages** get a hero + tile pair (4 Hardscape: `retaining-walls`, `fire-pits-features`, `driveways`, `outdoor-kitchens`; 6 Waterproofing: `basement-waterproofing`, `window-wells`, `crawl-spaces`, `concrete-raising`, `humidity-control`, `radon-mitigation`; 2 Trenchless: `handhole-pull-box`, `pipe-fusing`) and **2 division landings** get a hero-only asset (`/waterproofing/` → `src/assets/division/hero-waterproofing.jpg`, `/trenchless/` → `src/assets/division/hero-trenchless.jpg`) — exactly the B-15 `heroOut`/`heroOnly` pattern. **`sump-pumps` is untouched** (documented GAP after two sourcing passes — it keeps its `sprinkler-systems` alias and placeholder; flagged in the completion report per the phase's own dependency rule), and **`missile-boring` stays on the diagram track** (out of scope). The brief's nominal "15 pages / 13 services" therefore resolves to **14 pages / 12 services** — the delta is the sump-pumps GAP the brief itself anticipated.

**Hardscape overwrite + legacy preservation.** The 4 hardscape slugs' existing derivatives (`src/assets/service/hero-{slug}.jpg` + `tiles/{slug}.jpg`) are overwritten in place — but their originals are first preserved under `src/assets/service/legacy/` (+ `legacy/tiles/`) and **every non-service consumer is repointed to the legacy copies** so zero stock appears on any city hero, city card, project tile, or featured-project row (byte-identical proof by md5 vs. pre-phase baselines). Recon found the full consumer list is larger than planning knew: `LOCATION_HERO`/`LOCATION_CARD` for **oak-brook** (kitchens hero), **clarendon-hills** (fire hero), **burr-ridge** (driveways hero), **lombard** (walls hero), **plainfield** (walls tile), **yorkville** (fire tile); `PROJECT_LEAD` for **naperville-fire-court** (fire tile), **aurora-driveway-apron** (driveways tile), **lisle-retaining-wall** (walls tile); and — missed by planning recon — the **homepage trenchless division card** (`HomeAudienceEntries.tsx` imports `hero-driveways.jpg` directly), which is repointed to the legacy copy so the homepage stays byte-identical. `PROJECT_GALLERY`/`SERVICE_PROJECT` use separate `sp_*` files (unaffected); blog/resources use physical `public/` copies (unaffected).

**Resolution + alt pattern (B-14/B-15).** The 8 sourced gap services (6 waterproofing + 2 trenchless) drop their service-level `imageKey` aliases and resolve by slug via new `SERVICE_HERO`/`SERVICE_TILE` entries; nested per-project `imageKey`s inside featured-project entries are untouched (different field, different type). All 12 sourced services gain `photoAlt: {en, es}` — EN verbatim from the manifest, ES drafted this phase and logged in `Sunset-Services-TRANSLATION_NOTES.md` §B-16 as pending native review. Division-landing alts replace the existing aspirational values at `division.waterproofing.hero.alt` + `division.trenchless.hero.alt` in `messages/{en,es}.json` (keys already exist in both locales → EN/ES leaf parity unchanged); `DIVISION_HERO` gains `waterproofing` + `trenchless` entries resolved via the existing `DIVISION_HERO[division] ?? AUDIENCE_HERO[meta.heroImageKey]` fallback. Replace-by **2026-10-01** for all 14 assets (manifest).

**AudienceHero sizing fix (locked).** On the hero `<section>`, the fixed `sm:min-h-0 sm:h-[max(50vh,420px)] lg:h-[max(60vh,480px)]` becomes `sm:min-h-[max(50vh,420px)] lg:min-h-[max(60vh,480px)]` and the inline `maxHeight: '720px'` is removed (charcoal `backgroundColor` fallback stays). `overflow-hidden`, the absolute-fill image layer, gradients, and LCP discipline (no animation, `priority`) unchanged. Why: fixed height + the 720px cap + `overflow-hidden` clip the bottom-anchored CTA row on short/wide desktop viewports (operator repro at 1280×620); this extends the ratified M.10f/M.11c min-h-grow-with-content mobile fix to `sm`/`lg`. On very tall viewports the hero may exceed 720px when content demands — accepted; content safety wins.

**Kicker legibility (bounded).** The division kicker (colored `var(--audience-accent)`) gets the exact M.10f text-shadow treatment (`0 1px 2px rgba(0,0,0,0.62), 0 2px 10px rgba(0,0,0,0.45)`) at all breakpoints, checked on all 5 division landings over the final images. If any kicker still reads poorly, it is flagged to Chat — no further improvisation. No other visual change to the hero.

**Out of scope (locked):** `missile-boring`; `ServiceHero.tsx` and every other hero component; the homepage (beyond the byte-identical legacy repoint); Sanity schemas/content; new dependencies; secrets/env values; merging this branch (Goran verifies on Vercel Preview, then merges). Branch: `feat/b16-code-images-hero-fix` off `main` (post-B-16-Cowork merge).

**Logged by:** Code, 2026-07-04, before any source change (first commit of the Code leg).

---

## 2026-07-04 — Phase B-16 Code outcome: 12 of 14 wired; 2 sourced images rejected at verification

Appended after the code change + full verification, before the final commit. All items surfaced (not self-ratified):

- **Two sourced images REJECTED at Code-phase verification — readable, geolocating text (never-acceptable class).** (1) `stock-trenchless-division-landing-hero-01.jpg`: the back-turned worker's red shirt reads **"BARANGAY AYALA ALABANG / FIBER OPTIC PROJECT"** in large white print, crisply legible at hero scale (2400×1350) — it geolocates the scene to the Philippines; a second worker's face is turned toward the camera. (2) `stock-trenchless-handhole-pull-box-hero-01.jpg`: three legible **Russian** signs — "О ПОЖАРЕ ЗВОНИТЬ **101, 112**" (Russian emergency numbers), a "ПГ" hydrant marker, and a "ВЕДЕТСЯ ВИДЕОНАБЛЮДЕНИЕ" surveillance plaque. Both defects sit squarely in the plan-of-record's never-acceptable list ("visible brand logos/readable plates" — same class as B-15's rejected "GENGRAS" decal and "reads European" trucks) and were missed at cowork QA full-frame scale (a 3464-px frame shrinks the text; the 2400-px hero crop + desktop render restores it). **Handling: GAP-pattern, the same as sump-pumps** — `handhole-pull-box` keeps its `property-enhancement` placeholder alias (photoAlt removed), `/trenchless/` keeps its audience-alias hero and its previous `division.trenchless.hero.alt` in both locales; the two rejected derivatives were deleted and their optimizer entries removed (sources stay archived in `docs/stock-bridge/trenchless/` + the manifest). Wiring either back is 3 lines + one optimizer entry if Chat overrules. _Alternative rejected: wiring them and only flagging — puts Philippine/Russian signage on a DuPage launch if the Preview review misses it; the binding accuracy rule ("an inaccurate photo is worse than none") wins._
- **`missile-boring` aliased `imageKey: 'driveways'` (planning recon missed it).** Once `driveways` became a stock photo, missile-boring's page would silently show a stock paver driveway. Fixed by a `legacy-driveways` key in `SERVICE_HERO`/`SERVICE_TILE` (pointing at the preserved originals) + a one-line alias change — the page renders byte-identical, staying on its diagram track.
- **The homepage trenchless division card imports `hero-driveways.jpg` directly** (`HomeAudienceEntries.tsx`, also missed by planning recon) — repointed to `legacy/hero-driveways.jpg`; the homepage renders byte-identical.
- **Legacy preservation implemented as planned:** `src/assets/service/legacy/{hero-*,tiles/*}` holds the 4+4 pre-B-16 hardscape derivatives (md5-verified against pre-phase baselines); all six `LOCATION_HERO`/`LOCATION_CARD` city picks (oak-brook, clarendon-hills, burr-ridge, lombard, plainfield, yorkville) and the three `PROJECT_LEAD` entries repointed. Note: the three placeholder project pages (`naperville-fire-court`, `aurora-driveway-apron`, `lisle-retaining-wall`) no longer render anywhere (the `/projects` surface is Sanity-driven since B-11/M-10g; those slugs 404) — the `PROJECT_LEAD` repoints are defensive; a rendered-DOM sweep confirmed zero stock media on any project surface.
- **Kicker legibility (Decision-7 bounded check):** the M.10f text-shadow was applied to the kicker at all breakpoints. Over the final images: waterproofing/hardscape/trenchless/landscape read acceptably; **snow-removal still reads poorly** (dark-green accent over the dark-foliage band of the snowy street photo — a dark halo cannot rescue dark-on-dark) and **landscape remains the weakest of the passing four** (dark-green accent over bright-green shrubs). Per the locked decision, no further improvisation — both flagged to Chat (likely fix: a per-division lighter accent for the kicker, or a localized scrim).
- **No crop-position overrides.** All 12 wired photos keep their key subject under plain centre crops at hero and tile ratios (contact-sheet review of all derivatives). No `heroPosition`/`tilePosition` set.
- **Manifest change-log appended (append-only)** documenting the 12-of-14 integration and the two rejections; the B-16 Cowork tables were not edited.

**Verification result:** build 202/202 (0 TS errors), lint 0 errors, `tsc` 0, optimizer idempotent (re-run byte-identical; only expected deltas vs pre-phase baseline: 8 hardscape overwrites + new derivatives + legacy copies; every B-14/B-15 derivative and brand asset md5-identical). Hero matrix: 5 divisions × EN/ES × 6 viewports (incl. 1280×620) — breadcrumb, kicker, H1, subhead, both CTAs fully visible, ≥ 8 px breathing room, no horizontal overflow, 60/60 PASS. 13 target route-pairs (12 wired + waterproofing landing) × EN/ES: manifest-verbatim alt in live DOM **and** prerendered HTML; rejected images confirmed absent from prerendered HTML. City heroes/cards + missile-boring + homepage card resolve to legacy media (hash-distinct from stock); Wheaton EN/ES city grids carry the new tiles + photoAlt; sump-pumps untouched on its `sprinkler-systems` alias. EN/ES leaf parity 1406 = 1406. `validate:schema` 0/0 (24 URLs), `validate:seo` 0/0 (196 URLs), `test:consent` 23/23; a11y/links/mobile results recorded in the completion report.

**Logged by:** Code, 2026-07-04, after verification, before the final commit.

**Integration update (post-verification):** after the interim verification summary (including the two image rejections), **Goran instructed "push to main" in Chat, then repeated "just push to main now" without waiting for the still-running `validate:a11y`** — the B-15 handling otherwise repeated: every other harness green on `feat/b16-code-images-hero-fix`, then the branch fast-forwarded into `main` and pushed on the operator's explicit instruction. The a11y result is reported in Chat when it completes (fix-forward if it surfaces anything); the two rejected-image follow-ups + kicker flags remain open for Chat.

---

## 2026-07-04 — Phase Polish-01: division-image corrections (wiring-only) + About "The Team" section removed

Standalone polish batch (not a numbered B/M/P phase), executed on branch `polish/01-images-team-removal` — PR opened for Goran's review, **not merged**. Both decisions surfaced per the phase brief; full detail in `src/_project-state/Phase-Polish-01-Completion.md`.

**About "The Team" section removed entirely (both locales).** The Erick/Nick/Marcin cards only ever rendered placeholder initial-avatar tiles (EV/NV/M) — a placeholder surface that undercuts the premium first impression on the page whose whole job is trust. Removed: `AboutTeam.tsx` + `TeamCard.tsx` + `InitialsAvatar.tsx` (no other consumers), the `Person × 3` JSON-LD + `src/lib/schema/person.ts` (existed solely for those cards; `validate-schema.mjs` no longer requires `Person` on `/about/`), and the `about.team.*` strings (EN/ES). **Kept:** `src/data/team.ts` (the chat knowledge base still builds its Team digest from it) and all org-level identity — the sitewide `LocalBusiness`/`Organization` `@graph` and the founder/owner story (Nick → Erick) in the brand-story copy. _Alternative rejected: keeping the section until real portraits arrive — the portraits have been pending since M.03 with no ETA, and shipping placeholder avatars at launch is worse than shipping no team section. Downside accepted: the About page loses its §2.3 white/cream alternation at one seam (Brand Story and Credentials are both cream — visually clean, but the lock is no longer literal), and re-adding a team section later means rebuilding the cards (the deleted components live in git history)._

**Division-image fixes were wiring-only — no new binaries.** The homepage "Five divisions" cards and wizard Step-1 tiles for Waterproofing / Snow Removal / Trenchless were re-pointed to images already in the repo: the B-16/B-15 stock-bridge division heroes (`division/hero-waterproofing.jpg`, `division/hero-snow-removal.jpg` — the same assets their landings render, replace-by 2026-10-01 / 2027-01-31) and the preserved real trenching photo (`service/legacy/hero-driveways.jpg`). Alt text replaced in both locales (stock images use the generic manifest alts — no Sunset/city implication; the trenchless "golden hour" flourish corrected to describe the actual photo). _Alternative rejected: sourcing/generating better-fitting new images — re-opens the whole stock-sourcing pipeline (license/AI/text verification) for a correctness pass, and no real Sunset photos exist for these divisions yet (waterproofing has none; snow can't exist before the 2026–27 season). Downside accepted: two more visitor-facing surfaces now show temporary stock imagery until the replacement passes run — those passes must include these surfaces (same files as the landings, so the swap is automatic; only alts need a re-check)._

**Logged by:** Code, 2026-07-04, after verification, before the final commit.

**A11y-harness addendum (post-push):** the deferred `validate:a11y` never produced a trustworthy local result — one 2-hour 0%-CPU hang (killed), then three completed runs on the identical build returning 81 → 754 → 711 errors with failure sets that flip per URL between runs (`/` = Lighthouse 100 + axe 0 in one run, 52 errors in the next). Artifact classes: Lighthouse PROTOCOL_TIMEOUT/NO_FCP crashes, axe scanning the `#__next_error__` shell on pages the SEO (196-URL 0/0) and link (294-page 0-hard) validators prove healthy, and shared-chrome target-size/contrast clusters measured on degraded renders — none on a B-16 surface (the fixed audit sample excludes the 12 wired pages). Follow-up for Chat: one `validate:a11y` run from a healthy machine or CI against production, expected to restore the B-15 0/0 baseline. Logged by Code, 2026-07-04.

---

## 2026-07-12 — Phase M.08: delete-revalidation keyed on delta::operation(), not slug-absence (corrects PR #23's premise)

**PR #23's premise was empirically wrong, and is superseded.** PR #23 (`fix/delete-revalidation-slug-less-detail`, merged as PR #23 @ `40182de`) built the correct *mechanism* for purging a deleted post's detail page — the `{concrete, literal}` split in `revalidateForDocument` and the whole-route literal `/[locale]/{blog,resources,projects}/[slug]` — but wired it to the wrong *trigger*. It keyed the whole-route purge on **"the payload carries no slug,"** on the stated (never-verified) premise that *"the Sanity delete webhook payload carries no slug, so the detail-page path drops out of the revalidation list."* That premise is false. It shipped, deployed to production (Vercel `sunsetservices`, "Sunset Services Team" scope, `main` @ `addf709`), and **did not remove deleted posts from their own page** — a portal-deleted post kept serving a cached **200** at `/blog/<slug>`.

**What the evidence actually shows (gathered live 2026-07-12).** The Sanity delete webhook (project `i3fawnrl`, hook "Site revalidation") fires on delete and its payload **carries the slug**: a real portal-delete's attempt log returned `revalidatedPaths` including the concrete `/blog/portal-delete-revalidation-live-proof-2026-07-12` (both locales). The webhook projection is `{_type, _id, "slug": slug.current}` and `slug.current` resolves off the *pre-delete* document, so a delete looks identical to a publish in the payload — nothing in it says "this was a delete." Therefore `expandSlugDetail()` took its **with-slug branch** and emitted only the concrete path; PR #23's slug-absent literal branch was **dead code on this webhook**.

**The real defect: a concrete-URL `revalidatePath` does not evict an on-demand-generated dynamic page on Vercel; the whole-route route literal does.** In the same failing delete, `revalidatePath('/blog')` DID evict the index (the post vanished from `/blog` immediately) while `revalidatePath('/blog/<slug>')` did NOT evict the detail page (cached 200, minutes later). The difference: `/blog` is built at build time; `/blog/<slug>` for a portal-authored post is generated **on demand** (`dynamicParams = true` since M.02/PR #22) and lives in the full-route cache, which a concrete-URL `revalidatePath` does not purge. The whole-route literal `/[locale]/blog/[slug]` does (proven on `next start` in PR #23 §5: `x-nextjs-cache` HIT → MISS).

**Decision:** key the whole-route purge on **`operation === 'delete'`**, not on slug-absence. A new optional `operation` (`'create' | 'update' | 'delete'`) is threaded from the webhook body → `SanityRevalidationPayload` → both revalidate route handlers → `revalidateForDocument`; `expandSlugDetail()` emits the route literal whenever the event is a delete (regardless of whether it carries a slug), and stays surgical (concrete page only) for a publish. The signal comes from **`delta::operation()`** in the webhook's GROQ projection, which returns `"create" | "update" | "delete"` — the only thing in the payload that distinguishes a delete from a publish. PR #23's slug-absent literal branch is **kept** as a belt-to-the-braces fallback (if `slug.current` is ever null, the route is still purged).

**Backwards compatibility is mandatory and preserved.** `operation` is optional. The Sanity projection edit that adds `"operation": delta::operation()` is an **operator step** (manage.sanity.io) that may lag the code deploy; until it lands, `operation` is absent and revalidation is **byte-identical to pre-M.08 `main`** (concrete path when a slug is present, whole-route literal when it isn't). The code is safe to ship before the webhook is edited. `coerceSanityOperation()` narrows the untrusted field to a known value or `undefined` (the safe default), so a malformed/absent `operation` can never make a revalidation worse than before the signal existed.

**Not reverted:** PR #23's `{concrete, literal}` split and the route literal are correct and load-bearing — M.08 adds the trigger they were missing, it does not undo them. `dynamicParams` / `dynamic` / `revalidate` / `generateStaticParams` on every page are untouched (out of scope). Route literals must never pass through `withLocales()` (they already carry `[locale]`); no emitted path may contain `null`/`undefined` — both constraints are covered by the unit + integration tests.

**Operator step (required for the fix to take effect in production):** in manage.sanity.io → project "Sunset Services" (`i3fawnrl`) → API → Webhooks → "Site revalidation" → Edit → Projection, set:
```
...
## 2026-07-18 — Phase B.17: three deliberate schema omissions (aggregateRating, geo, sameAs) — omit-until-verified, never guess

**Context.** Phase B.17 (AI / answer-engine indexability — committed + pushed to `main` @ `a3f1d43`) expanded the sitewide JSON-LD `@graph` from ~600 bytes to a full `LocalBusiness` + `Organization` entity (22 cities, 34 services, Mon–Fri hours, Unilock credential, logo). Three high-value schema properties were deliberately **left out** rather than filled with the nearest available value. Each is a decision under the project truth rule (no invented or misrepresented facts, least of all in machine-readable form that Google and every LLM will cache), logged here because a future reader will otherwise read the gaps as oversights and "fix" them wrongly.

**Decision 1 — no `aggregateRating` on the `LocalBusiness` node.** The real 4.8★ / 38-review figure (`src/lib/constants/reviews.ts`, sourced from the Google Business Profile) is **not** emitted as schema.org `aggregateRating`. _Alternative rejected: emit it — the obvious "more schema = richer result" move. Rejected because Google's structured-data policy explicitly disallows **self-serving** `aggregateRating` (a business publishing a rating about itself): it is ineligible for rich results and is a documented manual-action (penalty) trigger._ Downside accepted: no star-rating rich snippet from our own markup; the rating still shows on the GBP, which is the surface Google trusts for it. This one is a permanent "never," not a "pending Erick."

**Decision 2 — `BUSINESS_GEO` is `null`; the builder omits `geo` entirely.** _Alternative rejected: reuse the Aurora **city** centroid already sitting in `src/data/locations.ts` (41.7606, −88.3201). Rejected because that is the municipal centroid — roughly 2 miles from the real address (1630 Mountain St) — and publishing it would put a wrong pin in the entity graph, contradicting the Google Business Profile._ Downside accepted: no `geo` node until the real coordinates are read off the GBP pin and set on `BUSINESS_GEO`; `buildSitewideGraph()` then picks them up with no other change. A wrong pin is worse than no pin (it degrades local-pack relevance).
...
## 2026-07-28 — B.17 offer catalog: every `Offer` carries the service URL; harness `mustNotHaveTypes` narrowed to page-level nodes

**Context.** Phase B.17 (`main` @ `a3f1d43`) shipped the sitewide `hasOfferCatalog` with schema verification listed as **pending** in its completion report — `npm run validate:schema` was never re-run. Run now, it was red with **818 errors across all 24 URLs**: 816 × `(Offer): missing required field "url"` (34 catalog Offers × 24 pages — each Offer carried `itemOffered.url` but nothing on the Offer node itself, which the harness's `REQUIRED_FIELDS` table requires and which the per-page builder `buildServiceSchema()` already satisfies), plus 2 × `forbidden @type "Service"` on `/request-quote/` + `/thank-you/` — a false positive: the only `Service` nodes on those routes are the ones B.17 legitimately embedded **inside the sitewide graph's catalog**, but the D14/D15 assertion matched the full recursive node index.

**Decision 1 — each catalog `Offer` emits `url`, same value as its `itemOffered` Service page.** The offer's landing page IS the service page; one URL, stated twice. _Alternative rejected: point `Offer.url` at the quote wizard (`/request-quote/?service=<slug>`) the way the per-service-page `buildServiceSchema()` does. Rejected because the catalog's whole purpose (B.17) is to give answer engines somewhere specific to **cite** — the service page — and 34 query-param wizard URLs in the sitewide `@graph` on every page would be noise, not signal._ Downside accepted: `url` is duplicated on `Offer` and `itemOffered` (a few hundred bytes per page of redundant JSON-LD).

**Decision 2 — `mustNotHaveTypes` now asserts against PAGE-LEVEL nodes only** (paths `block[N]` or `block[N].@graph[M]`), not the recursive index. The D14/D15 intent is "this **route** must not emit its own `Service`/`BreadcrumbList`/`WebPage`/`ItemList` block"; the sitewide graph is explicitly allowed on every page, and since B.17 it legitimately contains nested `Service` entities. _Alternatives rejected: (a) drop `'Service'` from the two routes' `mustNotHaveTypes` — weakens the guard (a page-level `Service` block on the quote/thank-you routes would then pass unnoticed); (b) strip the `Service` nodes out of the catalog — guts B.17's answer-engine purpose._ Downside accepted: a forbidden type nested inside a page-level block no longer flags on these routes — acceptable because the required-field checks still walk **all** nested nodes; only the type-presence assertion narrowed. Verified both ways: 0 errors / 0 warnings across 24 URLs after the fix, and a negative control (temporarily forbidding `BreadcrumbList` on `/landscape/`) still flagged exactly 1 error.

**Logged by:** Code, 2026-07-28, on branch `fix/schema-offer-url` (rebased onto `main` @ `a6986e6`); PR open — operator verifies on Vercel Preview + merges. Full detail in `src/_project-state/Hotfix-Schema-Offer-URL-Completion.md`.
