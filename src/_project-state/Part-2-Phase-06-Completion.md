...
- **Real lead-alert email to `info@sunsetservices.us`** is deferred to Phase 2.08. Resend is in sandbox mode (no verified domain) and only the account's verified owner address can receive — for the Sunset Services Resend account that address is `dinovlazar2011@gmail.com` (matches Phase 2.01 docs). User decision (logged in `Sunset-Services-Decisions.md` 2026-05-12): route `RESEND_TO_EMAIL=dinovlazar2011@gmail.com` for the Phase 2.06 dev window so the wiring is exercisable end-to-end; flip to `info@sunsetservices.us` in Phase 2.08 after domain verification + branded HTML template land. End-to-end smoke verified: a real wizard submission produces a Sanity `quoteLead` doc AND a real Resend email delivered to `dinovlazar2011@gmail.com` (smoke run on 2026-05-12 — Sanity doc `pJmrsKLAWC800vFHedAx9B`, Resend send accepted with no error in the server log).
...
10. **Resend `RESEND_TO_EMAIL` routed to `dinovlazar2011@gmail.com` for the Phase 2.06 dev window** (per user decision logged 2026-05-12). Phase 2.01 docs say the Resend account was created for Sunset Services on `dinovlazar2011@gmail.com` — that's still correct. The sandbox-mode restriction is the issue: until a domain is verified at Resend, the account can only send to its own verified owner address. So during the Phase 2.06 dev window we route the lead alert to `dinovlazar2011@gmail.com`, where the user can manually monitor for incoming leads; Phase 2.08 (which already covers branded HTML templates + domain verification) will flip `RESEND_TO_EMAIL` to `info@sunsetservices.us`. End-to-end smoke after the env update succeeded: Sanity doc `pJmrsKLAWC800vFHedAx9B` + Resend send to `dinovlazar2011@gmail.com` accepted (no error in server log; direct API test independently returned `{"id":"fbfc7e8e-c90e-4ced-bb26-f59e5007013b"}`). **Note:** an earlier smoke iteration's server log showed a "vertexcons1@gmail.com" verified-only error — this was traced to a stale `RESEND_API_KEY=re_BQr8dhss_...` in the dev machine's process env (different value from `.env.local`'s `re_W6RbEymY_...`); when the right key was forced into the running server via explicit shell env, the verified owner matched `dinovlazar2011@gmail.com` exactly as expected. The `re_BQr8dhss_...` key is from a different person's Resend account, same class of stale-env pollution as the Sanity vars (surprise #9). **Action item for the user:** unset both `RESEND_API_KEY` and the stale Sanity vars from the shell profile so future dev runs don't need explicit overrides.
...
- [x] Local smoke test: full happy-path submission produces one `quoteLead` doc in Sanity. Verified via direct Sanity API query: `Al3E26R37amzsHAqPBrGTo` (FullSmoke), `I3ROLIaQjmV1gZbnPXELVq` (Conv), and `pJmrsKLAWC800vFHedAx9B` (E2E with email sent successfully) all present.
- [x] Local smoke test: lead-alert email lands in `RESEND_TO_EMAIL`. **VERIFIED** with `RESEND_TO_EMAIL=dinovlazar2011@gmail.com` — server log shows no Resend error on the final happy-path submit; direct Resend API test independently returned `{"id":"fbfc7e8e-c90e-4ced-bb26-f59e5007013b"}` with HTTP 200. The email will flip to `info@sunsetservices.us` in Phase 2.08 (decision logged).
...
- [x] `RESEND_TO_EMAIL` received at least one test email during smoke testing. **VERIFIED** to `dinovlazar2011@gmail.com` (the Sunset Services Resend account's verified owner address). Flip to `info@sunsetservices.us` deferred to Phase 2.08 per the 2026-05-12 decision in `Sunset-Services-Decisions.md`.
...
1. ✅ **A real visitor submitting the wizard produces a `quoteLead` document in Sanity Studio AND a real email in `RESEND_TO_EMAIL`** — verified end-to-end. `RESEND_TO_EMAIL=dinovlazar2011@gmail.com` at Phase 2.06 (the Sunset Services Resend account's verified owner address while sandbox mode is active); flips to `info@sunsetservices.us` in Phase 2.08 per the 2026-05-12 decision.
...
- **`RESEND_TO_EMAIL` flip to `info@sunsetservices.us` deferred to Phase 2.08** (per the 2026-05-12 decision logged in `Sunset-Services-Decisions.md`). At Phase 2.06 the value is `dinovlazar2011@gmail.com` — the verified owner address on the Sunset Services Resend account. Phase 2.08 will verify the `sunsetservices.us` domain and flip the TO alongside shipping the branded HTML template + getting the FROM off the Resend sandbox sender.
- **Stale system env vars on the dev machine** silently routed early smoke writes to a different Sanity project AND a different Resend account. User should clean `NEXT_PUBLIC_SANITY_PROJECT_ID`, `SANITY_API_WRITE_TOKEN`, `SANITY_API_READ_TOKEN`, `NEXT_PUBLIC_SANITY_DATASET`, and `RESEND_API_KEY` (the `re_BQr8dhss_...` value — NOT the `.env.local` value) out of the shell profile so future smoke runs don't need explicit overrides.
